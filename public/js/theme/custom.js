	...
	if (jQuery("nav.nav_menu").length) {
		jQuery(".nav_menu > ul > li > a,.nav_menu > div > ul > li > a").on("click",function () {
			...
			if (li_menu.find(" > ul").length) {
				...
				jQuery(".kodcevap-main-wrap,.kodcevap-main-inner,.fixed-sidebar,.fixed_nav_menu").css({"height":"auto"});
				return false;
			}
		});
	}
...
jQuery.noConflict()(function discy_sidebar() {
	...
	if (jQuery('.menu_left').length && nav_menu_h > main_wrap_h) {
		...
		if (jQuery(".fixed_nav_menu").length) {
			jQuery('.kodcevap-main-wrap,.fixed_nav_menu').theiaStickySidebar({updateSidebarHeight: (jQuery(".widget-footer").length?false:true), additionalMarginTop: (jQuery("#wrap.fixed-enabled").length?jQuery(".hidden-header").outerHeight():0)+jQuery(".admin-bar #wpadminbar").outerHeight()+30});
		}
		if (jQuery(".fixed-sidebar").length) {
			jQuery('.kodcevap-main-wrap,.fixed-sidebar').theiaStickySidebar({updateSidebarHeight: (jQuery(".widget-footer").length?false:true), additionalMarginTop: (jQuery("#wrap.fixed-enabled").length?jQuery(".hidden-header").outerHeight():0)+jQuery(".admin-bar #wpadminbar").outerHeight()});
		}
	}
});
